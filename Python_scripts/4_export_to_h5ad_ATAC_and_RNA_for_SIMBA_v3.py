...
def load_r_matrix_to_csr(matrix_file: str, modality_name: str) -> tuple[csr_matrix, list]:
    ...
    with localconverter(ro.default_converter + pandas2ri.converter):
        ...
        # Transpose to (Cells x Features) and convert to CSR format (for faster row slicing)
...
def generate_peaks_bed(peak_features: list, output_dir: str):
    ...
    # Use 'str.split' for robust splitting
    split_parts = peaks_df['peak_id'].str.split('-', expand=True)
    if split_parts.shape[1] < 3:
        print("ERROR: Peak IDs do not seem to be in 'chr-start-end' format. Cannot generate BED file.", file=sys.stderr)
        return

    peaks_df[['chrom', 'start', 'end']] = split_parts.iloc[:, :3]
...
# --- Multi-Modal AnnData Assembly (FIXED) ---
...
def load_and_assemble_two_anndata(
    ...
    # Final Metadata Dtype Sanitization
    ...
    # Explicitly cast the shared cell barcode index to string
    ...
    # --- 5. Create Feature Metadata (FIXED) ---
    # RNA: Use gene names as index
    var_rna_df = pd.DataFrame(index=feature_names_rna)
    var_rna_df.index.name = 'gene_symbol'

    # ATAC FIX: Use the actual peak region names (genomic coordinates) as the index.
    # This reverses the "FINAL WORKAROUND" that caused the numeric index problem.
    var_atac_df = pd.DataFrame(index=feature_names_atac) # <-- CRITICAL FIX
    var_atac_df.index.name = 'peak_region'
    ...
    # ATAC: Use the CORRECTED var_atac_df with peak region names as the index
    atac_adata = ad.AnnData(X=X_atac, obs=obs_df.copy(), var=var_atac_df) # <-- This has the correct .var_names
